...
| `INKBITE-018` | Pull-request macOS and Windows CI exposed GNU-only archive/find flags, direct `.sh` execution, and external ZIP-tool assumptions in the source-package acceptance gate. | Release portability / NFR-012, SC-007 | `fixed` | macOS job `97061378278` failed on `tar --sort=name`; Windows job `97062279358` then exposed direct Bash-script execution and missing `zip`. Red `8418752`, green `4a0b63d` and `606f9da` now use deterministic Go tar/gzip/ZIP creation and ZIP mutation/extraction, portable shell inspection, and explicit Git Bash invocation. Focused release mutations, Darwin/Windows helper builds, full acceptance, and `make quality` pass. |

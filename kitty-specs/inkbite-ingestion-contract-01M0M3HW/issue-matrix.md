...
| `INKBITE-018` | Pull-request macOS and Windows CI exposed GNU-only archive/find flags, direct `.sh` execution, external ZIP-tool assumptions, MSYS path rewriting, newline-sensitive release mutations, drive-letter interpretation by `tar -f`, and a POSIX execute-bit check incorrectly applied to extracted Windows files. | Release portability / NFR-012, SC-007 | `fixed` | macOS job `97061378278` failed on `tar --sort=name`; Windows jobs `97062279358`, `97063000492`, `97063607737`, and `97064233208` then exposed direct Bash-script execution, missing `zip`, native-path rewriting, CRLF-vacuous warning mutation, `C:/...` being parsed as a remote tar host, and the impossibility of inferring archive POSIX modes from the Windows filesystem. Red `8418752` and `681f8aa`; green `4a0b63d`, `606f9da`, `3eb586a`, `ec325b8`, and `a0e2e4f` now use deterministic Go tar/gzip/ZIP creation, ZIP mutation/extraction, and direct tar/ZIP header mode inspection; portable shell inspection; explicit Git Bash invocation; bounded native/shell path seams; and mutation anti-vacuity. Focused release mutations, including executable archive mode rejection, Darwin/Windows helper builds, full acceptance, and `make quality` pass. |

Accounting: **18 findings; 14 fixed; 3 verified already fixed; 1 deferred;

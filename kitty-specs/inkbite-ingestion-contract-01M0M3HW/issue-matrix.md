...
| `INKBITE-018` | Pull-request macOS and Windows CI exposed GNU-only archive/find flags, direct `.sh` execution, external ZIP-tool assumptions, MSYS path rewriting, and newline-sensitive release mutations. | Release portability / NFR-012, SC-007 | `fixed` | macOS job `97061378278` failed on `tar --sort=name`; Windows jobs `97062279358` and `97063000492` then exposed direct Bash-script execution, missing `zip`, native-path rewriting, and CRLF-vacuous warning mutation. Red `8418752`; green `4a0b63d`, `606f9da`, and `3eb586a` now use deterministic Go tar/gzip/ZIP creation and ZIP mutation/extraction, portable shell inspection, explicit Git Bash invocation, a bounded native-path seam, and a mutation anti-vacuity assertion. Focused release mutations, Darwin/Windows helper builds, full acceptance, and `make quality` pass. |

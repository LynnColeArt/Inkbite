...
| `INKBITE-018` | Pull-request macOS CI exposed GNU-only `tar` creation and GNU `find` predicates in the otherwise portable source-package gate. | Release portability / NFR-012, SC-007 | `fixed` | macOS job `97061378278` failed on `tar --sort=name`; red `8418752` retained the platform contract and green `4a0b63d` replaced creation with deterministic Go `archive/tar`, `gzip`, and `archive/zip` output plus portable shell inspection. Focused release mutations, Darwin/Windows helper builds, full acceptance, and `make quality` pass. |

...
| `INKBITE-018` | Pull-request macOS and Windows CI exposed GNU-only archive/find flags, direct `.sh` execution, external ZIP-tool assumptions, MSYS path rewriting, newline-sensitive release mutations, and drive-letter interpretation by `tar -f`. | Release portability / NFR-012, SC-007 | `fixed` | macOS job `97061378278` failed on `tar --sort=name`; Windows jobs `97062279358`, `97063000492`, and `97063607737` then exposed direct Bash-script execution, missing `zip`, native-path rewriting, CRLF-vacuous warning mutation, and `C:/...` being parsed as a remote tar host. Red `8418752`; green `4a0b63d`, `606f9da`, `3eb586a`, and `ec325b8` now use deterministic Go tar/gzip/ZIP creation and ZIP mutation/extraction, portable shell inspection, explicit Git Bash invocation, bounded native/shell path seams, and mutation anti-vacuity. Focused release mutations, Darwin/Windows helper builds, full acceptance, and `make quality` pass. |
